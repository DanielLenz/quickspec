# Fitting formulae for linear matter power spectra in CDM + Baryon + Massive
# Neutrino (MDM) cosmologies, using the Eisenstein and Hu fitting formulae
...
class mps_lin_eihu(mps.mps_lin):
    def __init__(self, cosmo, deltaH=None, tilt=1, sigma8=None):
        """
        Linear matter power spectrum given by fitting function of Eisenstein
        and Hu (1999).
        ...
        z_drag = (
            1291. * omhh**(0.251) / (1. + 0.659 * omhh**(0.828)) *
            (1. + z_drag_b1 * obhh**(z_drag_b2)))
        y_drag = z_equality / (1. + z_drag)

        sound_horizon_fit = 44.5 * np.log(9.83 / omhh) / np.sqrt(
            1.0 + 10.0 * obhh**(0.75))

        # Set up for the free-streaming & infall growth function.
        p_c = 0.25 * (5.0 - np.sqrt(1. + 24. * f_cdm))
        p_cb = 0.25 * (5.0 - np.sqrt(1. + 24. * f_cb))

        # Compute small-scale suppression.
        alpha_nu = (
            f_cdm / f_cb * (5.0 - 2. * (p_c + p_cb)) / (5. - 4. * p_cb) *
            (1. + y_drag)**(p_cb - p_c) *
    ...
    def p_kz(self, kk, z):
        omega_denom = self.cosmo.oml + (1. + z)**2 * (
            self.cosmo.omm * (1. + z))
        omega_lambda_z = self.cosmo.oml / omega_denom
        omega_matter_z = self.cosmo.omm * (1. + z)**3 / omega_denom
        growth_k0 = (
            self.z_equality / (1. + z) * 2.5 * omega_matter_z /
            (omega_matter_z**(4. / 7.) - omega_lambda_z +
            (1. + omega_matter_z / 2.) * (1. + omega_lambda_z / 70.)))
        growth_to_z0 = self.z_equality * 2.5 * self.cosmo.omm / (
            self.cosmo.omm**(4. / 7.) - self.cosmo.oml +
            (1. + self.cosmo.omm / 2.) * (1. + self.cosmo.oml / 70.))
        ...
        if self.f_hdm != 0:
            y_freestream = (
                17.2 * self.f_hdm * (1. + 0.488 * self.f_hdm**(-7.0 / 6.0)) *
                (self.num_degen_hdm * qq / self.f_hdm)**2)
        temp1 = growth_k0**(1. - self.p_cb)
        temp2 = (growth_k0 / (1. + y_freestream))**(0.7)
        growth_cb = (1. + temp2)**(self.p_cb / 0.7) * temp1
        growth_cbnu = (
            self.f_cb**(0.7 / self.p_cb) + temp2)**(self.p_cb / 0.7) * temp1
        ...
        tf_sup_L = np.log(
            2.71828 + 1.84 * self.beta_c * self.alpha_gamma * qq_eff)
        ...
        return ((
            2997.0 * kk / self.cosmo.h)**(self.tilt + 3.) *
            self.cosmo.h**2 * (self.deltaH * tf_cb * growth_to_z0)**2 /
            kk**3 * (2. * np.pi)**2 * self.norm**2)
    ...
    def cobenorm(self):
        ...
        n = self.tilt - 1
        return 1.94e-5 * self.cosmo.omm**(
            -0.785 - 0.05 * np.log(self.cosmo.omm)) * np.exp(
                -0.95 * n - 0.169 * n * n)
