...
class mps_lin_bbks(mps.mps_lin):
    def __init__(self, cosmo, n=1., sigma8=0.84):
        """
        Linear matter power spectrum given by fitting function of Bardeen,
        Bond, Kaiser, Szalay (1986).
        ...
        self.gamma = cosmo.omm * cosmo.h * np.exp(
            -cosmo.omb * (1. + np.sqrt(2. * cosmo.h) / cosmo.omm))

        self.arr_z = np.array([
            0.0, 0.5, 1., 1.5, 2., 3., 4., 6., 8., 12., 16., 32., 64., 128.,
            256., 512., 1300.])
        self.arr_h = np.array([
