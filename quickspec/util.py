...
def enumerate_progress(list, label=''):
    """
    Version of python's builtin 'enumerate' iterator which displays a
    progress bar.
    ...
    for i, v in enumerate(list):
        ...
        if cpct > ppct:
            ...
            sys.stdout.write(
                "\r [" + ('%02d:%02d:%02d' % (dh, dm, ds)) + "] " +
                label + " " + int(10. * cpct / 100) * "-" + "> " +
                ("%02d" % cpct) + r"%")
            sys.stdout.flush()
...
def deriv(xv, yv=None):
    """
    Numerical differentiation based on 3-point lagrange interpolation,
    following the IDL 'DERIV' function.
...
def download(url, fname):
    ...
    def dlhook(count, block_size, total_size):
        ...
        if cpct > ppct:
            sys.stdout.write(
                "\r quickspec::util::download:: " + int(10. * cpct / 100) *
                "-" + "> " + ("%02d" % cpct) + r"%")
            sys.stdout.flush()
