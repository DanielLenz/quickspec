...
basedir = os.environ.get(
    'QUICKSPEC_DATA',
    os.path.dirname(qs.__file__) + "/data") + "/Bethermin_2011/"
...
class kern():
    ...
    def w_lxz(self, l, x, z):
        return 1. / (1. + z) * self.counts.jbar(
            self.nu, z, cosmo=self.cosmo, smax=self.smax)


class counts():
    def __init__(self, model="mean"):
        print(
            "quickspec::cib::bethermin_2011::counts:: loading " +
            model + "mean")
